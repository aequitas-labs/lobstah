...
| Stop hook (`lobstah man haul`) | On Claude Code, enforces a background watcher while work is in flight; `--park` restores a blocking wait. Inert unless the session holds the helm or is soaking (or the directory opts in with a `.lobstah-man` file or `LOBSTAH_MAN=1`). |
...
| ---- | --- | ------------ |
| Helm (orchestrator) | `/helm`, or `lobstah man helm` | Signs on as the one lobsterman for its grounds: prints the charter, then arms `man wait` as a background task. The Stop hook reminds if missing. `/relieve` steps down. |
