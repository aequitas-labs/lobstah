...
describe('lobItems (the crawling lobs on the spyglass page)', () => {
  it('walks one lob per attention item when lobs are on', () => {
    const items = lobItems(att, { lobs: true, preview: false,  });
  ...
  it('is empty when lobs are off', () => {
    expect(lobItems(att, { lobs: false, preview: false,  })).toEqual([]);
  });

  it('the ?lob preview works only while lobs are on', () => {
    expect(lobItems([], { lobs: true, preview: true,  }).map((i) => i.key)).toEqual(['preview']);
    expect(lobItems([], { lobs: false, preview: true,  })).toEqual([]);
    expect(lobItems([], { lobs: true, preview: false,  })).toEqual([]);
  });
  ...
  it('a pr:* item walks as a link to its PR with its kind label, not the modal', () => {
    ...
    expect(lobItems([{ id: 'c', lane: 'work', verb: 'pr:ready', kind: 'pr:ready', prUrl: 'x' }], { lobs: false, preview: false,  })).toEqual([]);
  });
  ...
  it('labels every kind the way the table and the pet do; a question has none', () => {
    ...
    expect(items[0]!.open).toEqual({ type: 'dispatch', key: 'work:q1' });
    expect(items[2]!.open).toBeUndefined();
  });
  ...
  it('acked items and this browser\'s hidden lobs do not walk; a new state hash re-shows a hidden one', () => {
    ...
    const shown = lobItems(att, { lobs: true, preview: false,  });
    expect(shown.map((i) => [i.key, i.hideKey, i.hideHash])).toEqual([['pr:draft:pr:o/r#1', 'pr:o/r#1', 'h1']]);
    expect(lobItems(att, { lobs: true, hidden: { 'pr:o/r#1': 'h1' }, preview: false,  })).toEqual([]);
    expect(lobItems(att, { lobs: true, hidden: { 'pr:o/r#1': 'h0' }, preview: false,  })).toHaveLength(1);
  });

  it('caps at four, the last one counting the rest', () => {
    const many = Array.from({ length: 6 }, (_, i) => ({ id: `id${i}`, lane: 'work', verb: 'blocked' }));
    const items = lobItems(many, { lobs: true, preview: false,  });
